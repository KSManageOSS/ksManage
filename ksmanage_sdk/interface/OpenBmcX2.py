...
class OpenBmcX2(Base):

    def get_url_info(self, key, name="CommonX2"):
        import yaml
        import os
    ...
    def create_psu(self,name, work_mode):
        return {
            "Name": name,
            "Oem": {"Public": {"WorkMode": work_mode}}
        }
    ...
    def setpsuconfig(self, client, args):
        ...
        if workmode == "Normal":
            ...
            if args.id is None or args.id == "":
                res.State('Failure')
                res.Message("-ID is required while set -M is not normal")
                return res
            if "," in args.id:
                ...
                id_list = args.id
            for id in id_list:
                patch_data['PowerSupplies'].append(self.create_psu(data_map.get(id), workmode))
            if workmode == "Active":
                standlist = list(set(id_list) ^ set(data_map.keys()))
                for id in standlist:
                    patch_data['PowerSupplies'].append(self.create_psu(data_map.get(id), 'Standby'))
            else:
